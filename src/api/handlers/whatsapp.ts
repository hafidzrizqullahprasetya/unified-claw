  ...
  try {
    ...
    if (parser.isMenuRequest(message.text)) {
      ...
      if (customer.length > 0) {
        await whatsAppService.sendMessage(
          customer[0].phone,
          "Maaf, saya tidak mengerti. Ketik *menu* untuk melihat katalog produk kami.",
        );
      }
    }
  } catch (error) {
    ...
    if (customer.length > 0) {
      try {
        await whatsAppService.sendMessage(
          customer[0].phone,
          "Terjadi kesalahan dalam memproses pesanan Anda. Silakan coba lagi nanti atau hubungi admin.",
        );
      } catch (innerError) {
